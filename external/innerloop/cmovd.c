...
extern int64_t innerloop60_transition(int64_t *f, int64_t *g, int64_t *v, int64_t *s);
...
void print_int51 (int64_t *g) {
  printf("0");
  for (int i = 0; i < limbs; i++) {
    printf("%+lld *(2** %d)", (long long) g[i], 51*i); 
  }
}
...
void inverse_255 (int64_t *f, int64_t *g, int64_t *uvrs) {
  int64_t s[5] = {0x0003224e016b148dLL, 0xfffd920bb830ab63LL,
		      0xfffe729511a76509LL, 0x0001ceced8666a39LL,
		      0x00018b076d64f596LL};
  int64_t v[5] = {0};
  //uint64_t cof[4] = {0x5b1b224e016b148dLL, 0x69d9422c905dc185LL,
  // 		     0x9db0ccd4719ca544LL, 0x18b076d64f59639dLL};
  //int64_t uvrs2[bigloops*4];
  int32_t i;
  int64_t m = -1;  // minusdelta
  //int64_t fuv, grs;

  //printf("V0="); print_int51(v);
  //printf("\nS0="); print_int51(s);
  //printf("\n");
  //for (i=1; i<=bigloops; i++) {
    //fuv = f[0] + (1LL<<urpos);
    ...
    //m = innerloop60_transition(m,f,g,v,s);
    innerloop60_transition(f,g,v,s);
    //m = innerloop60(uvrs,m,f,g);
    ...
    //transition(f,g,uvrs,v,s);
    //printf("f%d=",i); print_ints(f);
    //printf("\ng%d=",i); print_ints(g);
    //printf("\nV%d=",i); print_int51(v);
    //printf("\nS%d=",i); print_int51(s);
    //printf("\n");
    //printf("assert((V%d-(u*V%d+v*S%d))%%(2**255-19) == 0)\n",i,i-1,i-1);
    //printf("assert((S%d-(r*V%d+s*S%d))%%(2**255-19) == 0)\n",i,i-1,i-1);
    //uvrs += 4;
  ...
  fpmul25519(cof,uvrs2+16,g);
  fpcneg25519(g,f[0]);
  */
  g[0] = v[0]; g[1] = v[1]; g[2] = v[2]; g[3] = v[3]; g[4] = v[4];
}
...
int main () {
  ...
  //printf("Z=0x18b076d64f59639d9db0ccd4719ca54469d9422c905dc1855b1b224e016b148d\n");
  time_nothing();
  //time_innerloop60();
  //time_transition();
  //time_fpadd25519();
