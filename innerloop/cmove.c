...
void inverse_255 (int64_t *f, int64_t *g) {
  static const __attribute__((aligned(32)))
  int64_t table[36]={19LL, 19LL, 19LL, 19LL, 0LL, 19LL, 0LL, 19LL,
  ...
  __attribute__((aligned(32))) int64_t FVGS[9*4];
  int i;

  memset(FVGS,0,288);
  FVGS[0] = -19LL;  FVGS[3] = 1LL;  FVGS[32] = 0x00008000LL;

  for (i = 0; i < 9; i++) FVGS[2+4*i] = g[i];
  innerloop60_transition(FVGS,table);
  for (i = 0; i < 9; i++) f[i] = FVGS[1+4*i];
}
...
TIME(g[0] = -19; g[limbs-1] = (1<<topbits);
     for (j=1; j<limbs-1; j++) g[j] = 0;
     for (j = 0;j < 1000;++j) inverse_255(f,g);
     ,
     int64_t f[limbs];
     int64_t g[limbs];
     ,
