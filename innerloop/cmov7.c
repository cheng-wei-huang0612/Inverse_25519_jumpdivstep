...
extern void innerloop60(int64_t *out, int64_t m, int64_t f, int64_t g);
...
void time_nothing(void)
{
  ...
  printf("# nothing cycles:");
  for (i = 1;i < TIMINGS;++i)
    printf(" %lld",t[i]-t[i-1]);
  printf("\n");
}

void time_innerloop60(void)
{
  long long t[TIMINGS];
  int64_t out[7];
  int i;
  ...
  for (i = 0;i < TIMINGS;++i) {
    t[i] = cpucycles();
    innerloop60(out,0,0,0);
  }
  printf("# innerloop60 cycles:");
  for (i = 1;i < TIMINGS;++i)
    printf(" %lld",t[i]-t[i-1]);
  printf("\n");
}

void time_inverse_255(void)
{
  long long t[TIMINGS];
  int64_t f[limbs]; 
  int64_t g[limbs];
