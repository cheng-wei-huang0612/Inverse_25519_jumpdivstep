...
extern void inverse_256 (int64_t *f, int64_t *g, const int64_t *table, int64_t *interm); 


static const __attribute__((aligned(32)))
  int64_t table[72]={0x3FFFFFFFLL, 0x3FFFFFFFLL, 0x3FFFFFFFLL, 0x3FFFFFFFLL,
		     0x200000000LL, 0x200000000LL, 0x200000000LL, 0x200000000LL,
		     0x8000000000000000LL, 0x8000000000000000LL,
		     0x8000000000000000LL, 0x8000000000000000LL, 
		     0X7FFFFFFE00000000LL, 0X7FFFFFFE00000000LL,
		     0X7FFFFFFE00000000LL, 0X7FFFFFFE00000000LL,
		     271991105LL, 0LL,0LL,1LL, //
		     1061780019LL,0LL,0LL,0LL, //
		     881460155LL ,0LL,0LL,0LL, //
		     733428139LL ,0LL,0LL,0LL, //
		     1073741498LL,0LL,0LL,0LL, //
		     1073741823LL,0LL,0LL,0LL, //
		     1073741823LL,0LL,0LL,0LL, //
		     1073741823LL,0LL,0LL,0LL, //
		     65535LL     ,0LL,0LL,0LL, //
		     361279807LL};

int64_t interm[4*(limbs+1)*11];
...
TIME(g[0] = -19; g[limbs-1] = (1<<topbits);
     for (j=1; j<limbs-1; j++) g[j] = 0;
     for (j = 0;j < 1000;++j) inverse_256(f,g,table,interm);
     ,
     int64_t f[limbs];
     int64_t g[limbs];
     int64_t interm[4*(limbs+1)*11];
...
int main () {
  ...
  printf("p = 0"); 
  for (i=0; i<9; i++) printf("%+lld * 2**(%d)", table[16+4*i], 30*i);
  printf("\n");
  ...
  for (j=0; j<25; j++) {
    ...
    */
    ...
    printf("f0 = p\n"); 
    printf("U0,V0,R0,S0 = 1,0,0,1\n");	
    for (int i=1; i<=11; i++) {
      printf("u%d=%lld\n", i, (long long) interm[4*(limbs+1)*(i-1)+0]);
      printf("v%d=%lld\n", i, (long long) interm[4*(limbs+1)*(i-1)+1]);
      printf("r%d=%lld\n", i, (long long) interm[4*(limbs+1)*(i-1)+2]);
      printf("s%d=%lld\n", i, (long long) interm[4*(limbs+1)*(i-1)+3]);
      printf("U%d,V%d,R%d,S%d=u%d*U%d+v%d*R%d,u%d*V%d+v%d*S%d,r%d*U%d+s%d*R%d,r%d*V%d+s%d*S%d\n",i,i,i,i,i,i-1,i,i-1,i,i-1,i,i-1,i,i-1,i,i-1,i,i-1,i,i-1);

      printf("f%d = 0",i);
      for (k=0; k<limbs; k++) printf("%+lld * 2**(30*%d)", (long long) interm[4*(limbs+1)*(i-1)+4*(k+1)+0], k);
      printf("\n");
      printf("g%d = 0",i);
      for (k=0; k<limbs; k++) printf("%+lld * 2**(30*%d)", (long long) interm[4*(limbs+1)*(i-1)+4*(k+1)+2], k);
      printf("\n");
      printf("vv%d = 0",i);
      for (k=0; k<limbs; k++) printf("%+lld * 2**(30*%d)", (long long) interm[4*(limbs+1)*(i-1)+4*(k+1)+1], k);
      printf("\n");
      printf("ss%d = 0",i);
      for (k=0; k<limbs; k++) printf("%+lld * 2**(30*%d)", (long long) interm[4*(limbs+1)*(i-1)+4*(k+1)+3], k);
      printf("\n") ;
    }
    printf("print(U%d,V%d,R%d,S%d)\n",11,11,11,11);
