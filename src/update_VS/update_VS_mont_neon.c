...
void update_VS_mont(
  ...
  // clear
    ...
    for (int i = 0; i < 9; i++){
        ...
        vec_tmp[i+1] = vadd_u32(vec_tmp[i+1], vmovn_u64( vandq_u64(vec_prod, vec_2p30m1)));
        vec_prod = vshrq_n_u64(vec_prod, 30);
    }
    vec_tmp[9+1] = vadd_u32(vec_tmp[9+1], vmovn_u64( vandq_u64(vec_prod, vec_2p30m1)));
    ...
    for (int i = 0; i < 9; i++){
        // carry = tmp[i] >> 30;
        vec_carry = vshr_n_u32(vec_tmp[i+1], 30);
        // tmp[i] = tmp[i] & (((uint64_t)1<<30) -1);
        vec_tmp[i+1] = vand_u32(vec_tmp[i+1], vec_u32_2p30m1);
        // tmp[i+1] += carry; 
        vec_tmp[i+2] = vadd_u32(vec_tmp[i+2], vec_carry);
    }
    ...
    //            and tmp += l0 * P
    uint32x2_t vec_l1 = vmul_u32(vec_tmp[1], vec_M);
    ...
    vec_tmp[1] = vadd_u32(vec_tmp[1], vmovn_u64( vandq_u64(vec_prod, vec_2p30m1)));
    vec_prod = vshrq_n_u64(vec_prod, 30);
    ...
    vec_tmp[8+1] = vadd_u32(vec_tmp[8+1], vmovn_u64( vandq_u64(vec_prod, vec_2p30m1)));
    vec_prod = vshrq_n_u64(vec_prod, 30);
  ...
  // clear
    ...
    for (int i = 0; i < 8; i++)
    {
        vec_small_tmp = vadd_u32(vec_small_tmp, vec_tmp[i+2]);
        vec_small_tmp = vshr_n_u32(vec_small_tmp, 30);
    }
    vec_small_tmp = vadd_u32(vec_small_tmp, vec_tmp[8+2]);
    ...
    vec_tmp[0+2] = vadd_u32(vec_tmp[2], vand_u32(vec_reductionhat, vdup_n_u32(19)));
    vec_tmp[8+2] = vsub_u32(vec_tmp[10], vand_u32(vec_reductionhat, vdup_n_u32(32768)));
    ...
    vec_carry = vdup_n_u32(0);
    for (int i = 0; i<9; i++){
        vec_carry = vshr_n_u32(vec_tmp[i+2], 30);
        vec_tmp[i+2] = vand_u32(vec_tmp[i+2], vec_u32_2p30m1);
        vec_tmp[i+3] = vadd_u32(vec_tmp[i+3], vec_carry);
    }



    // Step [14]: tmp += 2x vec_uhat_rhat & (P-A)
    vec_tmp[2] = vadd_u32(vec_tmp[2] , vand_u32(vec_uhat_rhat, vec_u32_2p30m19));
    vec_tmp[2] = vsub_u32(vec_tmp[2] , vand_u32(vec_uhat_rhat, vec_V[0]));
    for (int i = 0 + 1; i < (9 - 1); i++)
    {
        vec_tmp[i+2] = vadd_u32(vec_tmp[i+2] , vand_u32(vec_uhat_rhat, vec_u32_2p30m1));
        vec_tmp[i+2] = vsub_u32(vec_tmp[i+2] , vand_u32(vec_uhat_rhat, vec_V[i]));
    }
    vec_tmp[8+2] = vadd_u32(vec_tmp[8+2] , vand_u32(vec_uhat_rhat, vec_u32_2p15m1));
    vec_tmp[8+2] = vsub_u32(vec_tmp[8+2] , vand_u32(vec_uhat_rhat, vec_V[8]));
    ...
    {   
        // borrow = tmp[i] >> 31;
        vec_borrow = vshr_n_u32(vec_tmp[i+2],31);
        // tmp[i+1] = tmp[i+1] - borrow;
        vec_tmp[i+1+2] = vsub_u32(vec_tmp[i+1+2], vec_borrow);
        // tmp[i+0] = tmp[i+0] + (borrow << 30);
        vec_tmp[i+0+2] = vadd_u32(vec_tmp[i+0+2], vshl_n_u32(vec_borrow,30));
    }
    ...
    vec_tmp[0+2] = vadd_u32(vec_tmp[0+2], vand_u32(vec_reductionhat, vdup_n_u32(19)));
    vec_tmp[8+2] = vsub_u32(vec_tmp[8+2], vand_u32(vec_reductionhat, vdup_n_u32(32768)));
    ...
    for (int i = 0; i<9; i++){
        ...
        vec_tmp[i+2] = vand_u32(vec_tmp[i+2], vec_u32_2p30m1);
        // tmp[i+1] += carry; 
        vec_tmp[i+1+2] = vadd_u32(vec_tmp[i+1+2], vec_carry);
    }


    // So fat, vec_tmp[2:11] stores [ u*V*R-1 mod P, r*V*R-1 mod P ]
    ...
    for (int i = 0; i < 9; i++){
        ...
        vec_tmp2[i+1] = vadd_u32(vec_tmp2[i+1], vmovn_u64( vandq_u64(vec_prod, vec_2p30m1)));
        vec_prod = vshrq_n_u64(vec_prod, 30);
    }
    vec_tmp2[9+1] = vadd_u32(vec_tmp2[9+1], vmovn_u64( vandq_u64(vec_prod, vec_2p30m1)));
    ...
    for (int i = 0; i<9; i++){
        // carry = tmp[i] >> 30;
        vec_carry = vshr_n_u32(vec_tmp2[i+1], 30);
        // tmp[i] = tmp[i] & (((uint64_t)1<<30) -1);
        vec_tmp2[i+1] = vand_u32(vec_tmp2[i+1], vec_u32_2p30m1);
        // tmp[i+1] += carry; 
        vec_tmp2[i+1+1] = vadd_u32(vec_tmp2[i+1+1], vec_carry);
    }
    ...
    // Step [9]: l1 = tmp[0] * M mod 2^30
    //            and tmp += l0 * P
    vec_l1 = vmul_u32(vec_tmp2[0+1], vec_M);
    vec_l1 = vand_u32(vec_l1 ,vec_u32_2p30m1);

    vec_prod = vdupq_n_u64(0);
    vec_prod = vmlal_u32(vec_prod, vec_u32_2p30m19, vec_l1 );
    vec_tmp2[0+1] = vadd_u32(vec_tmp2[0+1], vmovn_u64( vandq_u64(vec_prod, vec_2p30m1)));
    vec_prod = vshrq_n_u64(vec_prod, 30);
    ...
    vec_prod = vmlal_u32(vec_prod, vec_u32_2p15m1, vec_l1 );
    vec_tmp2[8+1] = vadd_u32(vec_tmp2[8+1], vmovn_u64( vandq_u64(vec_prod, vec_2p30m1)));
    vec_prod = vshrq_n_u64(vec_prod, 30);
    ...
    for (int i = 0; i < 8; i++)
    {
        vec_small_tmp = vadd_u32(vec_small_tmp, vec_tmp2[i+2]);
        vec_small_tmp = vshr_n_u32(vec_small_tmp, 30);
    }
    vec_small_tmp = vadd_u32(vec_small_tmp, vec_tmp2[8+2]);
    ...
    vec_tmp2[0+2] = vadd_u32(vec_tmp2[0+2], vand_u32(vec_reductionhat, vdup_n_u32(19)));
    vec_tmp2[8+2] = vsub_u32(vec_tmp2[8+2], vand_u32(vec_reductionhat, vdup_n_u32(32768)));
    ...
    vec_carry = vdup_n_u32(0);
    for (int i = 0; i<9; i++){
        vec_carry = vshr_n_u32(vec_tmp2[i+2], 30);
        vec_tmp2[i+2] = vand_u32(vec_tmp2[i+2], vec_u32_2p30m1);
        vec_tmp2[i+1+2] = vadd_u32(vec_tmp2[i+1+2], vec_carry);
    }
    ...
    vec_tmp2[0+2] = vadd_u32(vec_tmp2[0+2] , vand_u32(vec_vhat_shat, vec_u32_2p30m19));
    vec_tmp2[0+2] = vsub_u32(vec_tmp2[0+2] , vand_u32(vec_vhat_shat, vec_S[0]));
    for (int i = 0 + 1; i < (9 - 1); i++)
    {
        vec_tmp2[i+2] = vadd_u32(vec_tmp2[i+2] , vand_u32(vec_vhat_shat, vec_u32_2p30m1));
        vec_tmp2[i+2] = vsub_u32(vec_tmp2[i+2] , vand_u32(vec_vhat_shat, vec_S[i]));
    }
    vec_tmp2[8+2] = vadd_u32(vec_tmp2[8+2] , vand_u32(vec_vhat_shat, vec_u32_2p15m1));
    vec_tmp2[8+2] = vsub_u32(vec_tmp2[8+2] , vand_u32(vec_vhat_shat, vec_S[8]));
    ...
    {   
        // borrow = tmp[i] >> 31;
        vec_borrow = vshr_n_u32(vec_tmp2[i+2],31);
        // tmp[i+1] = tmp[i+1] - borrow;
        vec_tmp2[i+1+2] = vsub_u32(vec_tmp2[i+1+2], vec_borrow);
        // tmp[i+0] = tmp[i+0] + (borrow << 30);
        vec_tmp2[i+0+2] = vadd_u32(vec_tmp2[i+0+2], vshl_n_u32(vec_borrow,30));
    }
    ...
    for (int i = 0; i<9; i++) {
        vec_tmp[i] = vadd_u32(vec_tmp[i+2], vec_tmp2[i+2]);
    }
    vec_carry = vdup_n_u32(0);
